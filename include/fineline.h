...
typedef enum {
	...
	FINELINE_QUIT,		/* Exit the editor without saving the file */
	FINELINE_SAVE,		/* Save a file and exit the editor */
	FINELINE_EOT,		/* Expect no more lines (exit program) */
...
/* This stores an "image" of the current input.  It is generated by
 * fineline_image() and used by fineline_draw().  It is complex enough to
 * deserve a long-ish explanation.
 *
 * The current input is split into one or more terminal rows.  The row and style
 * fields are dynamically allocated arrays of row content; each element is also
 * a dynamically allocated array of chars and strings. The rowwidth field is a
 * dynamically allocated array of row widths (in character columns, not bytes)
 * so the drawing code knows whether it needs to clear the line.  The height
 * field is the number of rows (i.e., the dimension of row, style, and
 * rowwidth), and is the smaller of the terminal height or fineline_t's
 * viewport height.
 *
 * Inputs can use fewer rows; the usedrows field is used to track that.  Inputs
 * can also use more rows, in which case scrolling may be necessary.  The
 * toprow field tracks how many rows were scrolled off the top of the image.
 *
 * The thisrow and thiscol fields are used during generation of the image,
 * i.e. while copying text and styles from the current input into the image's
 * row and style arrays.  thisrow is an index into row and style, while thiscol
 * is used to make sure we don't make a line too wide for the terminal.  The
 * virtualcol field tracks the column number within the input lines ignoring
 * line wrap (as opposed to thiscol, which is the column number within the
 * current row, not line), mostly so we can expand tab character to the proper
 * number of spaces.  The rowpos field is the position within
 * img->row[img->thisrow] and img->style[img->thisrow] where we'll be copying
 * text next.  The rowsize field stores the currently allocated size for the
 * current row, so we can detect if we need to expand it.  (UTF-8 text can
 * potentially fit some very long byte sequences into a fixed width, due to
 * composing diacritics.  We can't assume that, say, 4 x the terminal width
 * will always be sufficient.)
 *
 * The cursorrow and cursorpos fields are used to store where the cursor should
 * be displayed within the image.  While copying text from the input to the
 * image, when we detect that we've reached fineline's idea of where the cursor
 * is, we copy thisrow and thiscol to cursorrow and cursorcol.  Note that
 * thisrow is relative to toprow, not the absolute row number within the input.
 */
typedef struct {
	...
	int	thisrow;	/* current row number, relative to toprow */
